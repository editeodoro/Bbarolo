...
int ConvertFluxUnits(int argc, char *argv[], std::string whichtype) {
    ...
    if (whichtype=="j2k" && makelower(bunit).find("k ")!=std::string::npos) {
    ...
    // Handling conversions of 0th moment map generated by BB
    if (b.find("*km/s")!=std::string::npos) endbunit += " * KM/S";
    if (b.find("*m/s")!=std::string::npos)  endbunit += " * M/S";
