...
#define wu 5E-29					// Conversion factor to W.U.
#define pi M_PI
...
void Galmod<T>::calculate() {
	...
	if (readytomod) {
		if (in->pars().isVerbose()) std::cout << " Modeling...";
		NHItoRAD();
		galmod();
	}
	else {
...
void Galmod<T>::normalize() {
	...
	if (!modCalculated) {
		std::cout << "GALMOD normalizing error: the model has not been ";
		std::cout << "calculated yet.\n";
		return; 
	}
...
void Galmod<T>::ringIO(Rings<T> *rings) {
	...
	*/
	...
	if (uradii[0]!=0 && empty) 
		r->radii.push_back(uradii[0]+r->radsep/2.0);
	else {
...
//	Get maximum number of velocity profiles that fits inside the databuffer.
//	Initialize the number of profiles that has been done.

//	For each data buffer a full model is calculated and only the part that
//	fits inside the data buffer is used. After having done this control
//	is returned here.
...
//				surface density of the clouds is constant over the area of
//				the ring.
...
///   radiation. (See Mihalas and Binney 'Galactic Astronomy'. p.489)
///   N.B.:
///   The columndensity of the HI is in units of (1E20 atoms per cm2).
///   Velocities are in M/S and frequencies in HZ.
///   Solid angle is in steradians.
///   Intensity is flux per one steradian solid angle. In GALMOD it is
///   defined as the flux per beam of one square arcminute defined in W.U..	

	for (int isubs=0; isubs<nsubs; isubs++) {
		double ddum=0;

        if (axtyp==2) {
            double labsubs = crval3+cdelt3*(isubs+1-crpix3);
            ddum=2*(K/wu)/(0.038475E-4*(labsubs*labsubs));
            ddum=ddum/fabs(cdelt3);
        }
        else if (axtyp==3) {
			double fsubs = crval3+cdelt3*(isubs+1-crpix3);
			double labsubs = C/fsubs;
			ddum=2*(K/wu)/(0.038475E-4*(labsubs*labsubs));
			ddum=ddum/fabs(cdelt3);
		}
		else if (axtyp==4) {
            ...
            double labsubs=C/fsubs;
            ddum=2*(K/wu)/(0.1823E-04*(labsubs*labsubs));
            ddum=ddum/fabs(cdelt3);
        } 
		cd2i[isubs]=ddum*pow((pi/(180.0*60.0)),2);
	}
