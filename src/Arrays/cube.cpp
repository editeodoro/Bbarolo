...
template <class T>
bool Cube<T>::readCube (std::string fname, bool printInfo) {
...
void Cube<T>::BlankMask (float *channel_noise, bool onlyLargest){
    ...
    // NEGATIVE MASK
    ////////////////////////////////////////////////////////////////////////
    else if (par.getMASK().find("NEGATIVE")!=std::string::npos) {
    ...
    ////////////////////////////////////////////////////////////////////////
        ...
        if (!fexists(filename) || !ma->readCube(filename,false)) {
            std::cerr << "\n ERROR: Mask " << filename
    ...
    // Enlarging the mask if requested
    size_t found = par.getMASK().find("ENLARGE");
    if (found!=std::string::npos) {
        ...
        for (int z=0; z<axisDim[2]; z++) {
            for (int y=0; y<axisDim[1]; y++) {
                for (int x=0; x<axisDim[0]; x++) {
                    if (tmask[nPix(x,y,z)] == 1) {
                        // Growing the mask by N pixels in xy directions
                        for (int dx = -N; dx <= N; ++dx) {
                            for (int dy = -N; dy <= N; ++dy) {
                                int nx = x + dx;
                                int ny = y + dy;
                                if (nx >= 0 && nx < axisDim[0] && ny >= 0 && ny < axisDim[1])
                                    mask[nPix(nx,ny,z)] = 1;
                            }
                        }
                    }
...
int Cube<T>::plotDetections(){
    ...
    pyf << "#########################################################################\n"
        ...
        << "  common = dict(origin='lower',aspect='auto',extent=[-10,10,-10,10],interpolation='nearest') \n"
        << std::endl
        << "  # Plotting intensity \n"
        ...
        << "  ysize, xsize = m0.shape \n"
        << "  n = max(ysize, xsize) \n"
        << "  pad_y = ((n - ysize) // 2, n - ysize - (n - ysize) // 2) \n"
        << "  pad_x = ((n - xsize) // 2, n - xsize - (n - xsize) // 2) \n"
        << "  m0 = np.pad(m0,(pad_y, pad_x),mode='constant',constant_values=np.nan) \n"
        << "  m1 = np.pad(m1,(pad_y, pad_x),mode='constant',constant_values=np.nan) \n"
        << "  m2 = np.pad(m2,(pad_y, pad_x),mode='constant',constant_values=np.nan) \n"
        << std::endl
