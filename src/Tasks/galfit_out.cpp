...
std::vector<std::string> Galfit<T>::writeScripts_Python() {
    ...
    pyf << "def extract_pv(cube, x, y, width=0, order=0): \n\n"
    ...
    pyf << "zmin:zmax+1,ymin:ymax+1,xmin:xmax+1] \n"
        ...
        << "if len(files_mod)==0: raise FileNotFoundError('ERROR: no model in output directory') \n\n"
        << std::endl
        << "def plot_pv(i,ax,x0,y0,theta,**kwargs):\n\n"
        ...
        << "  axes = np.ravel(axes)\n\n"
        << "  # Major axis\n"
        << "  s,x,y = plot_pv(0,axes[0],xcen_m,ycen_m,pa,)\n"
        ...
        << "  ax_m = fig.add_axes([bbox.x1+0.13,pos.y1-0.4,0.4,0.4])\n\n"
        << "  pm0 = np.pad(mom0,(pad_y, pad_x),mode='constant',constant_values=np.nan) \n"
        << "  pm1 = np.pad(mom1,(pad_y, pad_x),mode='constant',constant_values=np.nan) \n"
        << "  ax_m.tick_params(labelbottom=False,labelleft=False,top=False,bottom=False,left=False,right=False) \n"
        << "  ax_m.imshow(pm0,origin='lower',cmap='Greys',aspect='auto',interpolation='nearest',norm=norm0,extent=extm)\n"
        << "  ax_m.imshow(pm1,origin='lower',cmap='seismic',aspect='auto',interpolation='nearest',norm=norm1,extent=extm, alpha=0.4)\n"
        << "  ax_m.plot(xcen_m,ycen_m,'x',color='y',markersize=7,mew=1.5,zorder=10)\n"
        ...
        << "  ax_m.axhline(ycen_m,ls='dotted',c='k',alpha=0.2)\n"
        << "  ax_m.axvline(xcen_m,ls='dotted',c='k',alpha=0.2)\n"
        << "  ax_m.plot(x,y,'-',lw=1.5,c='#B22222') \n\n"
        ...
        << "  ax_m.scatter(x_rad,y_rad,c='gold',s=10,alpha=0.8) \n\n"
        << "  # Minor axis\n"
        << "  s,x,y = plot_pv(1,axes[1],xcen_m,ycen_m,pa+90)\n"
        << "  ax_m.plot(x,y,'--',lw=1.5,c='navy') \n"
    ...
    pyf << "# Beginning position-velocity plot \n"
        ...
        << "    ax.set_title('Slices')\n"
        << "    ax.plot(xcen_m,ycen_m,'x',color='yellow',markersize=7,mew=1.5,zorder=10) \n"
        << "    ax.xaxis.set_major_locator(mpl.ticker.MaxNLocator(nbins=5))\n"
        << "    ax.yaxis.set_major_locator(mpl.ticker.MaxNLocator(nbins=5))\n\n"
        ...
        << "    ax_m2.plot(xi,yi,'o',c='royalblue')\n"
